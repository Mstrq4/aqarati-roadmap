...
export function TermInfo({term}:{term:string}){
  ...
  return <span className="inline-flex align-middle">
    <button ref={triggerRef} type="button" onClick={()=>setOpen(true)} aria-label={`شرح المصطلح: ${term}`} aria-haspopup="dialog" aria-expanded={open} aria-controls={id} className="-my-1 grid h-11 w-11 shrink-0 place-items-center rounded-xl text-muted-foreground transition-colors duration-200 hover:bg-secondary hover:text-primary focus-visible:ring-2 focus-visible:ring-ring"><Info className="h-4 w-4"/></button>
